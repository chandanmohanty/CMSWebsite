...
import type { MediaApi, MediaFolderNode, MediaItem } from "@/lib/media-api";
...
function createMockApi(): MediaApi {
  let nextId = 100;
  let nextFolderId = 10;
  ...
  return {
    ...
    async upload(files, folderId) {
      const created = files.map((file): MediaItem => {
        ...
        return {
          ...
          type: file.type === "image/svg+xml" ? "svg" : isImage ? "image" : file.type.startsWith("video/") ? "video" : file.type.startsWith("audio/") ? "audio" : file.type === "application/pdf" ? "pdf" : "document",
          size: file.size,
          folder_id: folderId,
          source: "upload",
          conversions: null,
    ...
    async removeFolder(id) {
      const prune = (nodes: MediaFolderNode[]): MediaFolderNode[] => nodes.filter((n) => n.id !== id).map((n) => ({ ...n, children: prune(n.children ?? []) }));
